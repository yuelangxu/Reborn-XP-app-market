...
def req(method,path,body=None,timeout=20):
    ...
    with urllib.request.urlopen(r,timeout=timeout) as resp: payload=resp.read()
    return json.loads(payload or b'{}')

def wait_http(seconds=15):
    end=time.time()+seconds
    while time.time()<end:
        try:return req('GET','/status')
...
def find(sid,selector):
    ...
    if ELEMENT_KEY not in value:raise RuntimeError(f'element not found: {selector}: {value!r}')
    return value

def click(sid,element):req('POST',f'/session/{sid}/element/{element[ELEMENT_KEY]}/click',{})
def switch_frame(sid,element):req('POST',f'/session/{sid}/frame',{'id':element})
def parent_frame(sid):req('POST',f'/session/{sid}/frame/parent',{})

def key_actions(sid,text):
    actions=[]
    for ch in text:
        actions.append({'type':'keyDown','value':ch});actions.append({'type':'keyUp','value':ch})
...
def wait_text(sid,expected,seconds=15):
    end=time.time()+seconds;last=''
    while time.time()<end:
        last=model_text(sid)
        if last==expected:return last
        time.sleep(.1)
    raise RuntimeError(f'model text mismatch; expected={expected!r} last={last!r}')

def main():
    if len(sys.argv)!=2:raise SystemExit('usage: trusted-input-driver-v8.py <url>')
    proc=subprocess.Popen([os.environ.get('CHROMEDRIVER') or 'chromedriver','--port=9515','--log-level=WARNING'],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True)
    sid=None
    try:
        wait_http()
        caps={'capabilities':{'alwaysMatch':{'browserName':'chrome','pageLoadStrategy':'normal','goog:chromeOptions':{'args':['--headless=new','--no-sandbox','--disable-gpu','--disable-dev-shm-usage']}}}}
        created=req('POST','/session',caps);value=created.get('value') or {};sid=value.get('sessionId') or created.get('sessionId')
        if not sid:raise RuntimeError(f'could not create WebDriver session: {created!r}')
        req('POST',f'/session/{sid}/url',{'url':sys.argv[1]})
        ...
        wave1='abcde';key_actions(sid,wave1);wait_text(sid,wave1)
        wave2='fghijklmnopqrstuvwxyz0123456789abcdefghijklmno';key_actions(sid,wave2);expected=wave1+wave2;wait_text(sid,expected)
        time.sleep(.8)
        ...
        if execute(sid,"return document.activeElement&&document.activeElement.id;")!='focus-sink':
            raise RuntimeError('host control did not receive focus')
        time.sleep(.7)
        if execute(sid,"return document.activeElement&&document.activeElement.id;")!='focus-sink':
            raise RuntimeError('Writer stole focus back from host control')
        ...
        key_actions(sid,'你好连续输入');expected+='你好连续输入';wait_text(sid,expected)

        parent_frame(sid)
        stats=execute(sid,'return window.__rowTrustedStats;') or {}
        debug=execute(sid,'return window.ROW_DEBUG_STATE||null;') or {}
        if int(stats.get('trustedKeydown',0))<10:raise RuntimeError(f'too few trusted keydown events: {stats!r}')
        if int(stats.get('trustedInput',0))<10:raise RuntimeError(f'too few trusted input events: {stats!r}')
        ...
        if proc.stdout:
            ...
            if log.strip():print(log,file=sys.stderr)
